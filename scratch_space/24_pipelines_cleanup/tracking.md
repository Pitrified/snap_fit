...
- 2026-07-23 : phase 3 done (03_ingest_sheet.md -> done). Authored `pipelines/ingest_sheet.ipynb` fresh (D2) via NotebookEdit: 1 header markdown + 6 thin code cells, no `def`/`class` inline (D3). Flow mirrors the reference `ingest_green_sheet.py`: isolated imports cell, PHOTO_FP param (defaults to the gitignored greendemo_v1 straight capture, side shot commented), decode QR + `load_sheet_config_by_id` (BoardConfigNotFoundError -> friendly "run generate_board first"), `SheetAruco.load_sheet`, a label table, and a matplotlib overlay. For the overlay chose `piece.centroid_in_sheet` (the documented sheet-space accessor) on `sheet.img_orig` rather than piece-local `contour.cv_contour`, so coordinates line up without a translate step I couldn't eyeball headless. Verified: ruff + format clean; ran the code cells end-to-end on the real photo data/greendemo_v1/sheets/greendemo_p1_1x_straight.jpg -> decoded greendemo, mask enabled, 12 pieces, 12/12 labelled A1-D3 (matches the phase-5 green validation); nbstripout --verify exit 0; `make pipelines-check` green 2/2. Wired scaffolding: README index row (with the generate_board dependency noted), backlog "Sheet ingest" row candidate -> promoted. Not committed. In-kernel inline overlay left for the user to eyeball with the notebook open.
