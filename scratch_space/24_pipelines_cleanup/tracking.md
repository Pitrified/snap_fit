...
| 3  | `ingest_sheet` notebook                  | [03_ingest_sheet.md](03_ingest_sheet.md)  | done  |
| 4  | Retire green scratch scripts, repoint guide (D18) | [04_retire_green_scratch.md](04_retire_green_scratch.md) | done |
...
- 2026-07-23 : phase 2 done (02_generate_board.md -> done). Authored `pipelines/generate_board.ipynb` fresh (D2) via NotebookEdit (no kernel was open): 1 header markdown + 7 thin code cells, no `def`/`class` inline (D3). Confirmed the reference imports/signatures still resolve against current src (BoardImageComposer.compose/__init__ unchanged) and that background_preset is Literal["white","green","blue"] with _MASKED_PRESETS={"green","blue"}, so the mask-validation cell matches src. Verified: ruff clean on the notebook; `make pipelines-check` green (1/1, import cell isolated as designed); ran the code cells end-to-end in a headless process (not the notebook itself) - 2 boards composed 940x1340, PNGs + both config JSONs saved under the gitignored data/aruco_boards/greendemo (D13, confirmed check-ignore), QR round-trips on both, mask enabled mode=as_threshold. `make nbstrip` clean (notebook never executed in-place, no outputs). Wired scaffolding: README index row added, backlog "Board generation" row candidate -> promoted. Not committed. Note: full in-kernel run + inline board display is left for the user to eyeball with the notebook open in VS Code. Committed later as e625ebd.
- 2026-07-23 : phase 3 done (03_ingest_sheet.md -> done). Authored `pipelines/ingest_sheet.ipynb` fresh (D2) via NotebookEdit: 1 header markdown + 6 thin code cells, no `def`/`class` inline (D3). Flow mirrors the reference `ingest_green_sheet.py`: isolated imports cell, PHOTO_FP param (defaults to the gitignored greendemo_v1 straight capture, side shot commented), decode QR + `load_sheet_config_by_id` (BoardConfigNotFoundError -> friendly "run generate_board first"), `SheetAruco.load_sheet`, a label table, and a matplotlib overlay. For the overlay chose `piece.centroid_in_sheet` (the documented sheet-space accessor) on `sheet.img_orig` rather than piece-local `contour.cv_contour`, so coordinates line up without a translate step I couldn't eyeball headless. Verified: ruff + format clean; ran the code cells end-to-end on the real photo data/greendemo_v1/sheets/greendemo_p1_1x_straight.jpg -> decoded greendemo, mask enabled, 12 pieces, 12/12 labelled A1-D3 (matches the phase-5 green validation); nbstripout --verify exit 0; `make pipelines-check` green 2/2. Wired scaffolding: README index row (with the generate_board dependency noted), backlog "Sheet ingest" row candidate -> promoted. Committed as bc38271. User then caught a real bug: PHOTO_FP was a CWD-relative Path("data/...") that only resolved from the repo root, so the notebook raised FileNotFoundError under the VS Code kernel (CWD = pipelines/). Fixed to build from get_snap_fit_params().paths.data_fol (SnapFitPaths), verified by running the notebook from /tmp; committed 7f8364e. generate_board already used aruco_board_fol so it was unaffected.
- 2026-07-23 : phase 4 done (04_retire_green_scratch.md -> done). D18 carve-out from D4: git rm'd scratch_space/23_green_background/generate_green_board.py and ingest_green_sheet.py now that the pipelines exist and are validated; the rest of 23_green_background stays as feature history. Repointed docs/guides/green_background.md steps 1 and 3 at pipelines/generate_board.ipynb and pipelines/ingest_sheet.ipynb (kept the illustrative snippets, removed only the dead `python scratch_space/...` invocations). Grepped the repo: the only other live pointers to the deleted paths were the two "Freshest reference" cells in pipelines/backlog.md -> repointed to the pipeline paths; all remaining mentions are historical scratch plan/tracking records, left per D4. Updated docs/index.md (green_background last-updated 2026-07-23) and appended a docs/log.md entry. Not yet committed.
