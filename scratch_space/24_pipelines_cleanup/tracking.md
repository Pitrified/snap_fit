...
- The agent-notebook convention lives at repo level in `AGENTS.md` +
  ...
  experiments (D20); reaching Claude Code needs a `CLAUDE.md` shim (Q14 open).
...
Sub-plan files are intentionally not created yet (user hold): Q1-Q13 are folded
in; only Q14 (add a `CLAUDE.md` shim so repo instructions reach Claude Code) is
open, and it may be a spin-off rather than a phase here.
