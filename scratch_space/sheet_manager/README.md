...
def aruco_loader(path: Path) -> Sheet:
    # Use default detector + sheet config
    detector_cfg = ArucoDetectorConfig()
    sheet_cfg = SheetArucoConfig(detector=detector_cfg)
    return SheetAruco(sheet_cfg).load_sheet(path)
