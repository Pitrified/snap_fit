...
- The mask value floor is 100, not 40 (D18), measured on real captures: pieces
  lit by reflected board light reach V 42-61 and share the background hue, so
  a lower floor erodes them. Safe band 60-120.
- Compatibility is kept, no controlled break, no WARNING.md (D19).
- Boards are displayed on a screen and photographed, never printed (D20), so
  the D18 value floor is tuned on representative data, not provisional.
- Scale: ~100+ boards for a 1500-piece puzzle; the QR sheet_index plus the slot
  label is the manual tracking key (D21).
...
| 3  | HSV green-mask preprocess option       | [03_hsv_green_mask_preprocess_option.md](03_hsv_green_mask_preprocess_option.md) | done |
| 4  | Board config resolution at ingest      | [04_board_config_resolution_at_ingest.md](04_board_config_resolution_at_ingest.md) | done |
| 5  | Tests and compatibility decision gate  | [05_tests_and_compatibility_decision_gate.md](05_tests_and_compatibility_decision_gate.md) | done |
| 6  | Docs and dataset warnings (if needed)  | [06_docs_and_dataset_warnings_if_needed.md](06_docs_and_dataset_warnings_if_needed.md) | done |
...
- 2026-07-20 : added "grayscale" to .vscode cSpell.words to silence the spellcheck false positive across the plan docs.
- 2026-07-20 : implemented phase 3. Added SheetPreprocessConfig owning the previously hardcoded Sheet.preprocess params (D12, resolves the REFA comment); nested background_mask inside it (Q11) and removed the phase-1 top-level field; added the mode switch (D17: as_threshold / flatten_to_white) plus HSV range validation with a named InvalidHsvBoundsError (G3). Added compute_hsv_mask and paint_masked_white wrappers in image/process.py; threaded preprocess through SheetArucoConfig -> SheetAruco.load_sheet -> Sheet. Recolored the detector out-of-warp border green->magenta (D16). Updated the config-contract tests for nesting and added tests/puzzle/test_sheet_preprocess.py (disabled == baseline, both mask modes extract a piece on green, flatten_to_white == baseline on a white frame). Full suite 390 passed, ruff clean, pyright 0 errors. Marked phase 3 done. Committed as 52222f4.
- 2026-07-20 : implemented phase 4 library helpers in src/snap_fit/aruco/board_config_resolver.py: load_sheet_config_by_id (id -> data/aruco_boards/{id}/{id}_SheetArucoConfig.json, applies derivation, raises named BoardConfigNotFoundError on missing folder/file), derive_background_mask (Q12 precedence: green/blue preset with no explicit mask auto-enables it, explicit mask always wins). Added 9 unit tests (derivation both directions, loader hit applies derivation, missing id and old board-only folder both raise). SheetAruco/Sheet untouched (no disk-config code). Full suite 399 passed, ruff clean, pyright 0 errors. Deferred the .ipynb cell edits to phase 5 (only verifiable against real green data); marked phase 4 done for the library deliverable and recorded the exact wiring in the phase 4 and phase 5 docs. Committed as 996dd22.
- 2026-07-20 : started phase 5 (code-only half). Added scratch_space/23_green_background/generate_green_board.py, which produced a printable greendemo board set under data/aruco_boards/greendemo/ (2 PNGs + both config JSONs; saved SheetArucoConfig has the mask enabled via derive_background_mask; QR round-trips). Added ingest_green_sheet.py demonstrating the phase-4 ingest flow (decode QR -> load_sheet_config_by_id -> load_sheet), verified on the stand-in PNG. Added tests/aruco/test_green_board_pipeline.py: detector rectifies white+green boards (G6), and a green board with composited pieces extracts them under as_threshold, flatten_to_white, and the derived mask (synthetic set, D15/D17). Full suite 404 passed, ruff + pyright clean. Phase 5 now in progress; blocked on the user printing greendemo and taking real photos before real-photo validation, the mask-mode experiment on real data, and the compatibility decision.
- 2026-07-21 : validated phase 5 on six real captures (data/greendemo_v1/sheets, 2 layouts x 1x/2x/5x zoom x straight/side). All 6 decode the QR and detect 20/20 markers and rectify, so detection on green holds on real data (G6 closed). Found and fixed two real-world defects: min_area 80k was wrong for this board scale (pieces are 10k-16k px^2; generator now saves 5000), and the default mask value floor of 40 classified glare-lit piece pixels as background, eroding every piece ~60% and dropping one - raised the default lower_hsv V to 100 (D18, safe band 60-120 measured). Without the mask the straight captures collapse to a single 618k-px contour, confirming the feature is required, not cosmetic. D17 experiment: as_threshold and flatten_to_white are byte-identical on this set (dark pieces); as_threshold stays default. Final: 12/12 pieces with unique labels A1-D3 on all six photos through the plain ingest script with no overrides. Added a glare-lit-piece regression test. Compatibility kept, no WARNING.md (D19). Suite 405 passed, ruff clean. Phase 5 done.
- 2026-07-21 : user clarified the boards are displayed on a screen and never printed (D20), and that the workflow needs ~100+ boards for a 1500-piece puzzle with the QR sheet_index plus slot label as the manual tracking key (D21). This retires the phase 5 "not yet tested on paper" caveat: the screen captures are the real setup, so the D18 value floor is representative rather than provisional.
- 2026-07-21 : completed phase 6 (docs) using the repo docs-write procedure. No WARNING.md needed (D19 keep-compat), so the phase was docs-only. Added guides/green_background.md covering the screen-board workflow, generation at scale, QR-driven ingest, and the min_area / value-floor pitfalls. Updated puzzle/sheet (which was stale: it still documented the removed hardcoded threshold attribute and pitfall), puzzle/sheet_aruco, aruco/board, aruco overview, and config overview; refreshed docs/index.md rows and prepended a docs/log.md entry. 0 broken links; no mkdocs config exists so no nav change. All six phases now done.
